...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Register([FromBody] RegisterDto dto)
    {
        ...
        var user = new User { Username = dto.Username, Email = dto.Email, PasswordHash = BCrypt.Net.BCrypt.HashPassword(dto.Password), PreferredLanguage = dto.PreferredLanguage };
    ...
    public async Task<IActionResult> Login([FromBody] LoginDto dto)
    {
        ...
        var user = await _userRepo.GetByUsernameAsync(dto.Username) ?? await _userRepo.GetByEmailAsync(dto.Username);
    ...
    public IActionResult Me()
    {
        ...
        try
        {
            ...
            return Ok(new { username = principal.FindFirst(ClaimTypes.Name)?.Value, role = principal.FindFirst(ClaimTypes.Role)?.Value });
        }
    ...
    private string GenerateToken(string userId, string username, string role)
    {
        ...
        var claims = new[] { new Claim(ClaimTypes.NameIdentifier, userId), new Claim(ClaimTypes.Name, username), new Claim(ClaimTypes.Role, role) };
        var token = new JwtSecurityToken(issuer: _config["Jwt:Issuer"] ?? "DiscoverMadina", audience: _config["Jwt:Audience"] ?? "DiscoverMadinaUsers", claims: claims, expires: DateTime.UtcNow.AddDays(7), signingCredentials: creds);
    ...
    {
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_config["Jwt:Key"] ?? "DiscoverMadinaSecretKey2025!"));
        return new JwtSecurityTokenHandler().ValidateToken(token, new TokenValidationParameters { ValidateIssuer = true, ValidIssuer = _config["Jwt:Issuer"] ?? "DiscoverMadina", ValidateAudience = true, ValidAudience = _config["Jwt:Audience"] ?? "DiscoverMadinaUsers", ValidateLifetime = true, IssuerSigningKey = key }, out _);
    }
}
